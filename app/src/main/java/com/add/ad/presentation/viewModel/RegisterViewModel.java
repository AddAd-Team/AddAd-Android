...
public class RegisterViewModel extends BaseViewModel {
    ...
    @ViewModelInject
    public RegisterViewModel(AuthRepository authRepository, CompositeDisposable compositeDisposable, SharedPref sharedPref, @Assisted SavedStateHandle savedStateHandle) {
    ...
    public void clickSignUpNext() {
        ...
        Matcher m = p.matcher(userEmail.getValue());
        ...
        if (userEmail.getValue() != null && emailVerifyCode.getValue() != null && userPassword.getValue() != null && userPasswordCheck.getValue() != null) {
            if (m.matches()) {
                ...
                emailErrorEvent.setValue("이메일 형식이 일치하지 않습니다");
            }
        }
    }
        ...
        if (userEmail.getValue() != null) {
            compositeDisposable.add(
                    authRepository.requestVerifyCode(userEmail.getValue())
                            ...
                            .subscribe(it -> Log.d("인증코드 에러코드", String.valueOf(it.code())), it -> Log.d("인증 코드 보내기 실패", it.getMessage())));
        }
    }
